...
ATL::CComAutoCriticalSection g_cs;    // Critical section for thread-safe accessing error messages.
std::map<DWORD, WTL::CString> g_sErrorMsg; // Last error messages for each calling thread.
...
{  
  ...
  g_cs.Lock();
  DWORD dwThreadId = GetCurrentThreadId();
  std::map<DWORD, CString>::iterator itMsg = g_sErrorMsg.find(dwThreadId);
