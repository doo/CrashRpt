...
{ 
  ...
  DWORD crashSenderFileAttribs = GetFileAttributes(m_sPathToCrashSender+sCrashSenderName);
  if(crashSenderFileAttribs == INVALID_FILE_ATTRIBUTES || (crashSenderFileAttribs & FILE_ATTRIBUTE_DIRECTORY))
  {
  ...
  HMODULE hDbgHelpDll = LoadLibrary(m_sPathToDebugHelpDll+sDebugHelpDLL_name);
  // and check result
  if(!hDbgHelpDll)
  {
...
int CCrashHandler::Destroy()
{
  ...
  {
    ATL::CComCritSecLock<ATL::CComAutoCriticalSection> lock(m_csThreadExceptionHandlers);
