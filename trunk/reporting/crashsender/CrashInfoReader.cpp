...
BOOL ERIFileItem::GetFileInfo(HICON& hIcon, WTL::CString& sTypeName, LONGLONG& lSize)
{
  hIcon = NULL;
...
WTL::CString CErrorReportInfo::GetErrorReportDirName()
{
  return m_sErrorReportDirName;
}

WTL::CString CErrorReportInfo::GetCrashGUID()
{
  return m_sCrashGUID;
}

WTL::CString CErrorReportInfo::GetAppName()
{
  return m_sAppName;
}

WTL::CString CErrorReportInfo::GetAppVersion()
{
  return m_sAppVersion;
}

WTL::CString CErrorReportInfo::GetImageName()
{
  return m_sImageName;
}

WTL::CString CErrorReportInfo::GetEmailFrom()
{
  return m_sEmailFrom;
}

WTL::CString CErrorReportInfo::GetProblemDescription()
{
  return m_sDescription;
}

WTL::CString CErrorReportInfo::GetSystemTimeUTC()
{
...
WTL::CString CErrorReportInfo::GetExceptionModule()
{
...
WTL::CString CErrorReportInfo::GetExceptionModuleVersion()
{
...
WTL::CString CErrorReportInfo::GetOSName()
{
  return m_sOSName;
}
...
WTL::CString CErrorReportInfo::GetGeoLocation()
{
...
WTL::CString CErrorReportInfo::GetMemUsage()
{
  return m_sMemUsage;
}
...
ERIFileItem* CErrorReportInfo::GetFileItemByIndex(int nItem)
{
  ...
  // Look for n-th item
  std::map<WTL::CString, ERIFileItem>::iterator p = m_FileItems.begin();
...
BOOL CErrorReportInfo::GetPropByIndex(int nItem, WTL::CString& sName, WTL::CString& sVal)
{
  sName.Empty();
  sVal.Empty();
  ...
  // Look for n-th item
  std::map<WTL::CString, WTL::CString>::iterator p = m_Props.begin();
  for (int i = 0; i < nItem; i++, p++);
  sName = p->first;
  sVal = p->second;
  return TRUE;
...
// Method that retrieves a property by zero-based index.
BOOL CErrorReportInfo::GetRegKeyByIndex(int nItem, WTL::CString& sKeyName, ERIRegKey& rki)
{
  sKeyName.Empty();
  ...
  // Look for n-th item
  std::map<WTL::CString, ERIRegKey>::iterator p = m_RegKeys.begin();
  for (int i = 0; i < nItem; i++, p++);
  sKeyName = p->first;
  rki = p->second;
  return TRUE;
...
CCrashInfoReader::CCrashInfoReader()
{
  ...
  m_nJpegQuality = 0;
  m_ptCursorPos = WTL::CPoint(0, 0);
  m_rcAppWnd = WTL::CRect(0, 0, 0, 0);
  m_bAddVideo = FALSE;
  m_dwVideoFlags = 0;
  m_nVideoDuration = 0;
...
{ 
  ...
  else // We should look for pending error reports
  {
    ...
    WTL::CString sSearchPattern = m_sUnsentCrashReportsFolder + _T("\\*");
    WTL::CFindFile find;
    ...
    while(bFound)
    {
      ...
      {
        WTL::CString sErrorReportDirName = m_sUnsentCrashReportsFolder + _T("\\") + 
          find.GetFileName();
        WTL::CString sFileName = sErrorReportDirName + _T("\\crashrpt.xml");
...
int CCrashInfoReader::UnpackString(DWORD dwOffset, WTL::CString& str)
{
  ...
  str = WTL::CString((LPCTSTR)pStrData, wStrLen/sizeof(TCHAR));
...
int CCrashInfoReader::ParseFileList(TiXmlHandle& hRoot, CErrorReportInfo& eri)
{
  ...
  while(fi.ToElement()!=0)
  {
    ...
    if(pszDestFile!=NULL)
    {
      WTL::CString sDestFile = strconv.utf82t(pszDestFile);      
      ERIFileItem item;
...
int CCrashInfoReader::ParseRegKeyList(TiXmlHandle& hRoot, CErrorReportInfo& eri)
{
  ...
  while(fi.ToElement()!=0)
  {
    ...
    if(pszDestFile!=NULL && pszRegKey!=NULL)
    {
      ...
      WTL::CString sRegKey = strconv.utf82t(pszRegKey);
...
int CCrashInfoReader::ParseCrashDescription(WTL::CString sFileName, BOOL bParseFileItems, CErrorReportInfo& eri)
{
  strconv_t strconv;
  FILE* f = NULL; 
  ...
  {
    // Get directory name
    WTL::CString sReportDir = sFileName;
...
BOOL CCrashInfoReader::UpdateUserInfo(WTL::CString sEmail, WTL::CString sDesc)
{
...
BOOL CCrashInfoReader::AddUserInfoToCrashDescriptionXML(WTL::CString sEmail, WTL::CString sDesc)
{ 
  strconv_t strconv;

  TiXmlDocument doc;

  WTL::CString sFileName = m_Reports[0].m_sErrorReportDirName + _T("\\crashrpt.xml");
...
{   
  ...
  WTL::CString sFileName = m_Reports[nReport].m_sErrorReportDirName + _T("\\crashrpt.xml");
  ...
  { 
    ...
    {
      WTL::CString sDestPath = m_Reports[nReport].m_sErrorReportDirName + _T("\\") + FilesToAdd[i].m_sDestFile;
...
BOOL CCrashInfoReader::RemoveFilesFromCrashReport(int nReport, std::vector<WTL::CString> FilesToRemove)
{   
  strconv_t strconv;
...
{  
  WTL::CString sDate = Utility::GetINIString(m_sINIFile, _T("General"), _T("LastRemindDate"));
  if(sDate.IsEmpty())
    return FALSE;
...
{  
  WTL::CString sPolicy = Utility::GetINIString(m_sINIFile, _T("General"), _T("RemindPolicy"));
...
LONG64 CCrashInfoReader::GetUncompressedReportSize(CErrorReportInfo& eri)
{
  ...
  {   
    // Get file name
    WTL::CString sFileName = it->second.m_sSrcFile.GetBuffer(0);
    // Check file exists
    hFile = CreateFile(sFileName, 
      GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, NULL, NULL); 
    if(hFile==INVALID_HANDLE_VALUE)
...
HICON CCrashInfoReader::GetCustomIcon()
{
  ...
  if(!m_sCustomSenderIcon.IsEmpty())
  {
    ...
    WTL::CString sResourceFile;
    WTL::CString sIconIndex;
    int nIconIndex = 0;
...
WTL::CString CCrashInfoReader::GetErrorMsg()
{
  return m_sErrorMsg;
}

WTL::CString CCrashInfoReader::GetPersistentUserEmail()
{
