...
{ 
    ...
    for(i=0; i<pSender->GetCrashInfo()->GetReportCount(); i++)
    {
        ...
        WTL::CString sTotalSize = Utility::FileSizeToStr(eri->GetTotalSize());
...
LRESULT CResendDlg::OnTrayIcon(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
{
  ...
  // If user right-clicks the icon, display context menu.
    if(LOWORD(lParam)==WM_RBUTTONDOWN)
    {
        ...
        WTL::CMenu submenu = menu.GetSubMenu(2);

        strconv_t strconv;
        WTL::CString sShow = pSender->GetLangStr(_T("ResendDlg"), _T("PopupShow"));
        WTL::CString sExit = pSender->GetLangStr(_T("ResendDlg"), _T("PopupExit"));
...
LRESULT CResendDlg::OnListRClick(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
{
    ...
    GetCursorPos(&pt);
    WTL::CMenu menu = LoadMenu(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDR_POPUPMENU));  
    WTL::CMenu submenu = menu.GetSubMenu(5);

    strconv_t strconv;
    WTL::CString sSelectAll = pSender->GetLangStr(_T("ResendDlg"), _T("SelectAll"));
    WTL::CString sDeselectAll = pSender->GetLangStr(_T("ResendDlg"), _T("DeselectAll"));
    WTL::CString sDeleteSelected = pSender->GetLangStr(_T("ResendDlg"), _T("DeleteSelected"));
    WTL::CString sDeleteAll = pSender->GetLangStr(_T("ResendDlg"), _T("DeleteAll"));
...
LRESULT CResendDlg::OnOtherActions(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
  ...
  CErrorReportSender* pSender = CErrorReportSender::GetInstance();
    ...
    GetCursorPos(&pt);
    WTL::CMenu menu = LoadMenu(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDR_POPUPMENU));  
    WTL::CMenu submenu = menu.GetSubMenu(3);

    WTL::CString sRemindLater = pSender->GetLangStr(_T("ResendDlg"), _T("PopupRemindLater"));
    WTL::CString sNeverRemind = pSender->GetLangStr(_T("ResendDlg"), _T("PopupNeverRemind"));
...
{   
  CErrorReportSender* pSender = CErrorReportSender::GetInstance();
    ...
    int nProgressPct = 0;
    std::vector<WTL::CString> messages;
  ...
  int nCurItem = FindListItemByReportIndex(pSender->GetCurReport());
    ...
    { 
      ...
      // Determine window mirroring flags (language specific).
            DWORD dwFlags = 0;
      WTL::CString sRTL = Utility::GetINIString(pSender->GetCrashInfo()->m_sLangFileName, _T("Settings"), _T("RTLReading"));
            if(sRTL.CompareNoCase(_T("1"))==0)
      ...
      // Display message box.
            WTL::CString sCaption = pSender->GetLangStr(_T("ProgressDlg"), _T("DlgCaption"));
            WTL::CString sTitle;
...
LRESULT CResendDlg::OnReportSizeChanged(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    // Update list
    int nItem = FindListItemByReportIndex(nReport);
    WTL::CString sTotalSize = Utility::FileSizeToStr(pSender->GetCrashInfo()->GetReport(nReport)->GetTotalSize());
    m_listReports.SetItemText(nItem, 1, sTotalSize);
