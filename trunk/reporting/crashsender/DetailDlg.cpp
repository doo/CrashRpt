...
LRESULT CDetailDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    WTL::CString sRTL = pSender->GetLangStr(_T("Settings"), _T("RTLReading"));
    ...
    // Set up header text
    WTL::CStatic statHeader = GetDlgItem(IDC_HEADERTEXT);
...
void CDetailDlg::FillFileItemList()
{
    ...
    for (i = 0; i<nCount; i++)
    {
        ...
        WTL::CString sDestFile = pfi->m_sDestFile;
        WTL::CString sSrcFile = pfi->m_sSrcFile;
        WTL::CString sFileDesc = pfi->m_sDesc;
        HICON hIcon = NULL;
        WTL::CString sTypeName;
...
LRESULT CDetailDlg::OnItemChanged(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/)
{
    ...
    if (lpItem->uChanged & LVIF_STATE && lpItem->uNewState & LVIS_SELECTED)
    {
        SelectItem(iItem);
    }
...
LRESULT CDetailDlg::OnItemDblClicked(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/)
{
    ...
    // Get file name
    WTL::CString sFileName = pFileItem->m_sSrcFile;
...
LRESULT CDetailDlg::OnExport(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ...
    WTL::CString sFileName = pCrashInfo->GetReport(m_nCurReport)->GetCrashGUID() + _T(".zip");

    // Display "Save File" dialog.
    WTL::CFileDialog dlg(FALSE, _T("*.zip"), sFileName,
    ...
    if(result==IDOK)
    {
        ...
        WTL::CString sExportFileName = dlg.m_szFileName;
...
LRESULT CDetailDlg::OnPreviewRClick(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
{
    ...
    WTL::CMenu submenu = menu.GetSubMenu(1);
    MENUITEMINFO mii;
    ...
    strconv_t strconv;
    WTL::CString sAuto = pSender->GetLangStr(_T("DetailDlg"), _T("PreviewAuto"));
    WTL::CString sText = pSender->GetLangStr(_T("DetailDlg"), _T("PreviewText"));
    WTL::CString sHex = pSender->GetLangStr(_T("DetailDlg"), _T("PreviewHex"));
    WTL::CString sImage = pSender->GetLangStr(_T("DetailDlg"), _T("PreviewImage"));
    WTL::CString sVideo = pSender->GetLangStr(_T("DetailDlg"), _T("PreviewVideo"));
    WTL::CString sEncoding = pSender->GetLangStr(_T("DetailDlg"), _T("Encoding"));
    ...
    {
        WTL::CMenuHandle TextEncMenu = submenu.GetSubMenu(6);
...
LRESULT CDetailDlg::OnListRClick(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
{
    ...
    WTL::CPoint pt;
    GetCursorPos(&pt);
    WTL::CMenu menu = LoadMenu(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDR_POPUPMENU));
    WTL::CMenu submenu = menu.GetSubMenu(6);

    strconv_t strconv;
    WTL::CString sOpen = pSender->GetLangStr(_T("DetailDlg"), _T("Open"));
    WTL::CString sDeleteSelected = pSender->GetLangStr(_T("DetailDlg"), _T("DeleteSelected"));
    WTL::CString sAttachMoreFiles = pSender->GetLangStr(_T("DetailDlg"), _T("AttachMoreFiles"));
...
LRESULT CDetailDlg::OnPopupOpen(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
  ...
  // Open the file with shell-provided functionality
  DWORD_PTR dwRet = (DWORD_PTR)::ShellExecute(0, _T("open"), sFileName, 0, 0, SW_SHOWNORMAL);
  ATLASSERT(dwRet > 32);
  return 0;
}
...
LRESULT CDetailDlg::OnPopupDeleteSelected(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
  ...
  std::vector<WTL::CString> asFilesToRemove;
  int i;
  for(i=0; i<m_list.GetItemCount(); i++)
    {
...
LRESULT CDetailDlg::OnPopupAddMoreFiles(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
  ...
  if(result==IDOK)
  {
    ...
    std::vector<WTL::CString> asItems;
    WTL::CString sItem;
    int i;
    for(i=0; ; i++)
    {
    ...
    {
      // Single file to add
      WTL::CString sFileName = asItems[0];
    ...
    else if(asItems.size()>1)
    {
      ...
      {
        WTL::CString sFileName = asItems[0]+_T("\\")+asItems[j];
