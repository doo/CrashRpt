...
LPBYTE CFileMemoryMapping::CreateView(DWORD dwOffset, DWORD dwLength)
{
  ...
  ATL::CComCritSecLock<ATL::CComAutoCriticalSection> lock(m_csLock);
...
BOOL CImage::IsImageFile(WTL::CString sFileName)
{
  FILE* f = NULL;
...
BOOL CImage::Load(WTL::CString sFileName)
{
  Destroy();
...
HBITMAP CVideo::DecodeFrame(BOOL bFirstFrame, WTL::CSize& FrameSize, int& nDuration)
{
...
BOOL CFilePreviewCtrl::SetFile(LPCTSTR szFileName, PreviewMode mode, TextEncoding enc)
{
  ...
  ATL::CComCritSecLock<ATL::CComAutoCriticalSection> lock(m_criticalSection);
...
PreviewMode CFilePreviewCtrl::DetectPreviewMode(LPCTSTR szFileName)
{
  ...
  std::set<WTL::CString> aTextFileExtensions;
  WTL::CString sExtension;
...
void CFilePreviewCtrl::SetEmptyMessage(WTL::CString sText)
{
  m_sEmptyMsg = sText;
}
...
//
WTL::CString CFilePreviewCtrl::FormatHexLine(LPBYTE pData, int nBytesInLine, ULONG64 uLineOffset)
{
  WTL::CString sResult;
  WTL::CString str;
  int i;
...
{	
  ...
  WTL::CString str = FormatHexLine(ptr, nBytesPerLine, nLineNo*m_nBytesPerLine );
...
{ 
  ...
  {
    WTL::CMemoryDC memDC(hDC, ps.rcPaint);
