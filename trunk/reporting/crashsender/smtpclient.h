...
public:
  ...
  WTL::CString GetRecipientAddress(int nRecipient);
  ...
  WTL::CString GetAttachment(int nAttachment);
...
private:
    ...
    WTL::CString m_sSubject;   // Subject
    WTL::CString m_sFrom;      // Sender address
    std::vector<WTL::CString> m_aTo; // Recipient addresses    
    WTL::CString m_sText;      // Message body
    std::vector<WTL::CString> m_aAttachments; // The list of file attachments
};
...
public:
    ...
    int SetSmtpServer(WTL::CString sServer, int nPort);
    ...
    // Returns zero on success, otherwise non-zero.
    int SendEmailAssync(CEmailMessage* msg,  AsyncNotification* scn);
...
protected:
    ...
    // Returns zero on success, otherwise non-zero.
    int ResolveSmtpServerName(LPCTSTR szEmailAddress, std::map<WORD, WTL::CString>& host_list);

    // Sends E-mail message to the specified SMTP server.
    // Returns zero on success, otherwise non-zero.
    int SendEmailToRecipient(WTL::CString sSmtpServer, CEmailMessage* msg);
  
    // Validates E-mail address syntax.
    // Returns zero on success, otherwise non-zero.
    int CheckAddressSyntax(WTL::CString addr);
    ...
    // Returns zero on success, otherwise non-zero.
    int CheckAttachmentOK(WTL::CString sFileName);

    // Encodes the given file into BASE-64 encoding and returns the file contents as a string.
    // Returns zero on success, otherwise non-zero.
    int Base64EncodeAttachment(WTL::CString sFileName, 
    ...
    WTL::CString m_sServer;       // SMTP server.
    int m_nPort;             // Port.
    WTL::CString m_sLogin;        // Login name (used for SMTP authentication).
    WTL::CString m_sPassword;     // Password (used for SMTP authentication).
    CEmailMessage* m_msg;    // Pointer to E-mail message being sent.
    AsyncNotification* m_scn; // Synchronization object.
};
