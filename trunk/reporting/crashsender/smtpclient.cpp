...
WTL::CString CEmailMessage::GetSubject()
{
  return m_sSubject;
}
...
WTL::CString CEmailMessage::GetSenderAddress()
{
...
{
  std::vector<WTL::CString> aAddresses = Utility::ExplodeStr(szEmailAddresses, _T(", ;"));
  size_t i;
...
WTL::CString CEmailMessage::GetRecipientAddress(int nRecipient)
{
...
WTL::CString CEmailMessage::GetAttachment(int nAttachment)
{
  // Return n-th attachment.
  return m_aAttachments[nAttachment];
}

WTL::CString CEmailMessage::GetText()
{
  return m_sText;
}
...
int CSmtpClient::SetSmtpServer(WTL::CString sServer, int nPort)
{
...
int CSmtpClient::SendEmailAssync(CEmailMessage* msg,  AsyncNotification* scn)
{
...
int CSmtpClient::SendEmail(CEmailMessage* msg)
{
  m_scn->SetProgress(_T("Start sending email"), 0, false);
    ...
    std::map<WORD, WTL::CString> host_list;
  ...
  // For each resolved domain name of SMTP server, try to send E-mail to the specified domain.
    std::map<WORD, WTL::CString>::iterator it;
...
int CSmtpClient::ResolveSmtpServerName(LPCTSTR szEmailAddress, std::map<WORD, WTL::CString>& host_list)
{
    ...
    if(r==0)
    {
      ...
      // Look for MX record
        ...
        // Save domain name to our list
                WTL::CString sServerName = WTL::CString(apResult->Data.MX.pNameExchange);
...
int CSmtpClient::SendEmailToRecipient(WTL::CString sSmtpServer, CEmailMessage* msg)
{
    ...
    WTL::CString sBodyTo; //Vojtech: Lines of the "To:" and "Cc:" lines, that will become part of the e-mail header.
    int iResult = -1;  
    WTL::CString sPostServer;
    WTL::CString sServiceName;
    SOCKET sock = INVALID_SOCKET;
    WTL::CString sMsg, str;
    std::set<WTL::CString>::iterator it;
    WTL::CString sStatusMsg;
    ...
    sServiceName.Format(_T("%d"), m_sServer.IsEmpty()?25:m_nPort);
    
    // Prepare message text (we need to replace "\n" by "\r\n" and remove . from message). 
    WTL::CString sMessageText = msg->GetText();
    ...
    {
        WTL::CString sFileName = msg->GetAttachment(i);
    ...
    // Add a message to log
    sStatusMsg.Format(_T("Getting address info of %s port %s"), sSmtpServer, WTL::CString(sServiceName));
    m_scn->SetProgress(sStatusMsg, 1);
    ...
    // Open socket
      ...
      // Add a message to log
        sStatusMsg.Format(_T("Connecting to SMTP server %s port %s"), sSmtpServer, WTL::CString(sServiceName));
        m_scn->SetProgress(sStatusMsg, 1);
  ...
  // Check return code (expect code 250)
    if(res==250)
    {
    sStatusMsg = WTL::CString(_T("EHLO command not supported"));
    bESMTP = true;
  }
  ...
  if(!bESMTP)
  {
    ...
    {
      // Add a message to log
      sStatusMsg = WTL::CString(_T("HELO command not supported"));
  ...
  // Send Content-Type
    ...
    {    
        WTL::CString sFileName = msg->GetAttachment(i);
        sFileName.Replace('/', '\\');
        WTL::CString sDisplayName = sFileName.Mid(sFileName.ReverseFind('\\')+1);
...
int CSmtpClient::GetMessageCode(LPSTR msg)
{
  ...
  std::vector<WTL::CString> aLines = Utility::ExplodeStr(WTL::CString(msg), _T("\n"));
  // Take the last line
  WTL::CString sLastLine = aLines[aLines.size()-1];
...
int CSmtpClient::CheckAddressSyntax(WTL::CString addr)
{
...
{		
  ...
  for(;;)
  {
    ...
    // Add a message to log
    WTL::CString sStatusMsg = WTL::CString(pszResponse);
    m_scn->SetProgress(sStatusMsg, 0);    
...
int CSmtpClient::CheckAttachmentOK(WTL::CString sFileName)
{
...
int CSmtpClient::Base64EncodeAttachment(WTL::CString sFileName, 
                                        std::string& sEncodedFileData)
{
