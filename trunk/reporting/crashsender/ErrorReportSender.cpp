...
BOOL CErrorReportSender::Init(LPCTSTR szFileMappingName)
{
  ...
  WTL::CString sRTL = Utility::GetINIString(m_CrashInfo.m_sLangFileName, _T("Settings"), _T("RTLReading"));
...
BOOL CErrorReportSender::InitLog()
{
  ...
  WTL::CString sLogDir = m_CrashInfo.m_sUnsentCrashReportsFolder + _T("\\Logs");	
  ...
  std::set<WTL::CString> asLogFiles;

  BOOL bFound = FileFinder.FindFile(sLogDir + _T("\\*.txt"));
  while(bFound)
  {
    WTL::CString sFile = FileFinder.GetFileName();
  ...
  {
    std::set<WTL::CString>::iterator it = asLogFiles.begin();
    WTL::CString sLogFile = *it;
    asLogFiles.erase(it);
...
WTL::CString CErrorReportSender::GetErrorMsg()
{
  return m_sErrorMsg;
}
...
void CErrorReportSender::SetExportFlag(BOOL bExport, WTL::CString sExportFile)
{
...
void CErrorReportSender::GetCurOpStatus(int& nProgressPct, std::vector<WTL::CString>& msg_log)
{
...
BOOL CErrorReportSender::TakeDesktopScreenshot()
{
  ...
  {
    WTL::CString sFileName = ssi.m_aMonitors[i].m_sFileName;
    WTL::CString sDestFile;
...
{   
  ...
  WTL::CString sMinidumpFile = m_CrashInfo.GetReport(m_nCurReport)->
    GetErrorReportDirName() + _T("\\crashdump.dmp");
  std::vector<ERIFileItem> files_to_add;
  ERIFileItem fi;
  WTL::CString sErrorMsg;
  ...
  if(hDbgHelp==NULL)
  {
    ...
    const WTL::CString sDebugHelpDLL_name = "dbghelp.dll";
  ...
  if(!bWriteDump)
  {    
    WTL::CString sMsg = Utility::FormatErrorMsg(GetLastError());
...
void CErrorReportSender::AddElemToXML(WTL::CString sName, WTL::CString sValue, TiXmlNode* root)
{
  strconv_t strconv;
...
BOOL CErrorReportSender::CreateCrashDescriptionXML(CErrorReportInfo& eri)
{
  BOOL bStatus = FALSE;
  ERIFileItem fi;
  WTL::CString sFileName = eri.GetErrorReportDirName() + _T("\\crashrpt.xml");
  WTL::CString sErrorMsg;
  strconv_t strconv;
  TiXmlDocument doc;
  FILE* f = NULL; 
  WTL::CString sNum;
  WTL::CString sCrashRptVer;
  WTL::CString sOSIs64Bit;
  WTL::CString sExceptionType;
...
{ 
  BOOL bStatus = FALSE;
  WTL::CString str;
  WTL::CString sErrorReportDir = m_CrashInfo.GetReport(m_nCurReport)->GetErrorReportDirName();
  WTL::CString sSrcFile;
  WTL::CString sDestFile;
  ...
  for(i=0; i<eri->GetRegKeyCount(); i++)
  {
    ...
    WTL::CString sFilePath = eri->GetErrorReportDirName() + _T("\\") + rki.m_sDstFileName;
...
BOOL CErrorReportSender::CollectFilesBySearchTemplate(ERIFileItem* pfi, std::vector<ERIFileItem>& file_list)
{
  ...
  {		
    ...
    WTL::CString sDir = pfi->m_sSrcFile.Mid(0, nPos);
...
int CErrorReportSender::DumpRegKey(WTL::CString sRegKey, WTL::CString sDestFile, WTL::CString& sErrorMsg)
{
  strconv_t strconv;
...
int CErrorReportSender::DumpRegKey(HKEY hParentKey, WTL::CString sSubKey, TiXmlElement* elem)
{
  strconv_t strconv;
  HKEY hKey = NULL;
  ...
  {    
    ...
    WTL::CString sKey = sSubKey.Mid(0, nSkip);
  ...
  else
  {
      ...
      else
      {
        ...
        if(lResult==ERROR_SUCCESS)
        {
          ...
          { 
            ...
            if(lResult==ERROR_SUCCESS)
            {
              DumpRegKey(hKey, WTL::CString(szName), key_node.ToElement());
            }
            ...
            if(lResult==ERROR_SUCCESS)
            {
              ...
              {                
                ...
                for(;;)
                {
                  if(szValues[pos2]==0)
                  {
                    WTL::CString sValue = WTL::CString(szValues+prev, pos2-prev);
    ...
    {      
      WTL::CString sErrMsg = Utility::FormatErrorMsg(GetLastError());
...
int CErrorReportSender::CalcFileMD5Hash(WTL::CString sFileName, WTL::CString& sMD5Hash)
{
...
{ 
  ...
  std::map<WTL::CString, ERIFileItem>::iterator it;
  FILE* f = NULL;
  WTL::CString sMD5Hash;
  ...
  { 
    ...
    WTL::CString sDstFileName = pfi->m_sDestFile.GetBuffer(0);
    // Define source file name
    WTL::CString sFileName = pfi->m_sSrcFile.GetBuffer(0);
    // Define file description
    WTL::CString sDesc = pfi->m_sDesc;
...
int CErrorReportSender::Base64EncodeAttachment(WTL::CString sFileName, WTL::CString& sEncodedFileData)
{
  int uFileSize = 0;
  ...
  // BASE-64 ecode data
  sEncodedFileData = base64_encode(uchFileData, uFileSize).c_str();
...
WTL::CString CErrorReportSender::FormatEmailText()
{
  WTL::CString sFileTitle = m_sZipName;
  sFileTitle.Replace('/', '\\');
...
{  
  ...
  if (m_CrashInfo.GetReport(m_nCurReport)->GetEmailFrom().IsEmpty()) 
  {
    ...
    WTL::CString copy((LPCTSTR)m_CrashInfo.m_sEmailTo, m_CrashInfo.m_sEmailTo.GetLength());
    TCHAR   separators[] = _T(";, ");
    TCHAR  *context		 = 0;
  ...
  if(pos>=0)
    sFileTitle = sFileTitle.Mid(pos+1);
  sFileTitle += _T(".md5");
  WTL::CString sTempDir;
  Utility::getTempDirectory(sTempDir);
  WTL::CString sTmpFileName = sTempDir +_T("\\")+ sFileTitle;
  FILE* f = NULL;
...
{  
  ...
  // The copy string will be modified by strtok.
  WTL::CString copy		 = m_CrashInfo.m_sEmailTo;
  TCHAR   separators[] = _T(";, ");
  TCHAR  *context		 = 0;
  TCHAR  *to			 = _tcstok_s(const_cast<LPTSTR>((LPCTSTR)copy), separators, &context);	
  while (to != 0) 
  {
    m_MapiSender.AddRecipient(_T("SMTP:")+WTL::CString(to));
...
WTL::CString CErrorReportSender::GetLangStr(LPCTSTR szSection, LPCTSTR szName)
{
...
WTL::CString CErrorReportSender::GetLogFilePath()
{
